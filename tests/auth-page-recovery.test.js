...
test('auth page recovery detects fraud_guard retry page state', () => {
  const state = {
    clickCount: 0,
    pageText: '糟糕，出错了！ 验证过程中出错。请重试。 错误代码：fraud_guard',
  ...
  const snapshot = api.getAuthTimeoutErrorPageState({
    pathPatterns: [/\/log-in\/password(?:[/?#]|$)/i],
  });

  assert.equal(Boolean(snapshot), true);
  assert.equal(snapshot.signupRestartCurrentAttemptBlocked, true);
});
