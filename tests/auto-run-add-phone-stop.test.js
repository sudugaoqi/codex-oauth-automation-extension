...
test('auto-run controller restarts the same final round when add-phone would otherwise stop the whole run', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  assert.equal(events.runCalls, 2, 'final-round add-phone stop should restart the same round once and then succeed');
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'retrying'), true, 'global-exit fallback should enter retrying phase');
  assert.equal(events.accountRecords.length, 0, 'eventual fallback success should not persist a failed round record');
  assert.ok(events.logs.some(({ message }) => /原本将结束全部轮执行，现直接重开当前轮/.test(message)));
  assert.equal(runtime.state.autoRunActive, false);
  assert.equal(runtime.state.autoRunSessionId, 0);
});
