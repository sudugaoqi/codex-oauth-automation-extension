...
- 展示一个单独的“接码”开关、注册方式 `signupMethod` 与“接码平台”下拉；接码平台当前支持 HeroSMS / 5sim / SMSBower / NexSMS。普通模式下开启接码后可把注册方式切到手机号注册，并在 OAuth 登录链路命中手机号登录验证码页时继续复用同一手机号续跑短信验证
...
- 注册方式 `signupMethod`
- 接码开关、接码平台 `phoneSmsProvider`，以及 HeroSMS / 5sim / SMSBower / NexSMS 各自的 API Key、国家/地区、价格上限和平台扩展设置
- iCloud 相关偏好
- LuckMail API 配置
- 自动运行默认配置
...
3. 手机号验证共享流程会按当前 sidepanel 中保存的接码平台 `phoneSmsProvider` 选择 HeroSMS、5sim、SMSBower 或 NexSMS，并使用对应平台的 API Key、国家/地区、价格上限申请或复用号码
4. 后台提交号码后，会按 `currentPhoneActivation.provider` 分发后续查码、完成、取消、ban、reuse：HeroSMS 走原 `getStatus / setStatus` 逻辑，5sim 走 `/v1/user/check|finish|cancel|ban|reuse`，SMSBower 走 `handler_api` 兼容的 `getNumber / getStatus / setStatus`
...
- HeroSMS、5sim、SMSBower 都归入接码平台适配层；默认平台是 HeroSMS，5sim 产品码固定为 `openai`，SMSBower 的 OpenAI service code 固定为 `dr`，5sim operator 默认 `any`。
