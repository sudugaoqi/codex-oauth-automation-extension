...
- `AGENTS.md`：仓库协作者指南，概述目录结构、常用测试命令、代码风格、提交流程与安全注意事项。
- `LICENSE`：项目许可证文件。
...
- `openspec/config.yaml`：OpenSpec 工作流配置，当前使用 `spec-driven` schema。
- `openspec/changes/add-smsbower-phone-provider/.openspec.yaml`：SMSBower 接码平台接入 change 的 OpenSpec 元数据。
- `openspec/changes/add-smsbower-phone-provider/proposal.md`：说明为何要新增 SMSBower 接码 provider、能力范围与影响面的 proposal。
- `openspec/changes/add-smsbower-phone-provider/design.md`：SMSBower provider 的技术设计文档，记录正式 API 域名、OpenAI service code、动态国家拉取和不支持能力边界。
- `openspec/changes/add-smsbower-phone-provider/tasks.md`：SMSBower provider 实施任务拆解清单。
- `openspec/changes/add-smsbower-phone-provider/specs/smsbower-phone-provider/spec.md`：SMSBower 接码 provider 的需求规格，定义 provider 选择、动态国家、拿号、轮询、收尾与不支持路径约束。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  展示 `OAUTH / 回调 / 总状态` 三块真实运行态信息，同时把“接码”开关与“验证码重发”拆成同一行展示；只有开启接码后，下面的 HeroSMS / 5sim / SMSBower / NexSMS 平
...
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、共享验证码自动重发次数配置与广播接收，并装
  ...
  窗当前额外提供“出现手机号验证”按钮，用于直接走与真实 add-phone 一致的 fatal 分支；HeroSMS 与 SMSBower 国家列表
  会在 sidepanel 初始化时按当前 provider 拉取并恢复到本地保存的国家/API 设置，接码开关关闭时会直接隐藏相关配置；账号记录快照同步改为默认自动模式，只
